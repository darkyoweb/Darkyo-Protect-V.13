  ...
  const member = message.author
    ...
    let time = args[0];
        ...
        const tempMuteFormatErr = new Discord.MessageEmbed()
          .setDescription('Error! Vous devez indiquer une durée pour votre rappel !.  \`[+rappel [Durée] [Raison]\`')
          .setColor('RED')
        if (!time) return message.channel.send(tempMuteFormatErr)

        const noReasonInput = new Discord.MessageEmbed()
          .setDescription('Error! Veuillez indiquer votre raison de rappel ! \`[+rappel [Durée] [Raison]\`')
          .setColor('RED')
