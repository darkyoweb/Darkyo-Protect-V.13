...
module.exports.run = async(bot,message,args) => {
  ...
  if (!user)
    return message.channel.send(
      `:x: Vous n'avez pas mentionné un utilisateur, ou vous avez donné un identifiant invalide !`
    );
  ...
  user.user
    .send(emebd1)
    .catch(() => message.channel.send(":x: __cet utilisateur n'a pas pu être DM !__"))
