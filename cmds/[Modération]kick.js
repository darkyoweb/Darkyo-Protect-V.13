...
module.exports.run = async(bot,message,args) => {
  ...
  if(member.id == "911610558682394655") return message.channel.send("_Je ne peux pas **kick** mon maître, désolé..._")
  ...
  if(kickedUser.hasPermission('BAN_MEMBERS')) {
    ...
    if (!Channel)
      return message.channel.send(
        ```:x:`` **Il n'y a pas de canal dans cette guilde qui s'appelle**\`logs\`\n**Merci d'en crée un !**`
      );
