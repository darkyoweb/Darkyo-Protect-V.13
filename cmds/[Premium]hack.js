...
module.exports.run = async(bot, message, args) => {
    ...
    const channel = message.guild.channels.cache.find(c => c.id === '1003618243069620274'); //Votre bot doit être sur le serveur.
    ...
    if(!db.get('Premium').find({ user_id: author }).value()) {
        message.channel.send("Vous ne possédez pas le __**\"Premium du Bot\"**__, <a:Nop:836606008964415529> \n\nfaite ``+premium`` pour savoir comment l'avoir ! <a:couronne:836754425133006918>").catch(console.error); 
    }
    ...
    if(member.id === message.author.id) return message.channel.send('*Bruh, vous ne pouvez pas vous auto-hack !*');
        ...
        message.channel
        .send(`**Commencement du __Hack__ de : **${member},** en cours, veuillez patientez !** <a:patient1:964187920623874078>`)
        .then(async (msg) => {
            setTimeout(async function () {
                await msg.edit("Récupération du Token de l'utilisateur en cours : ")
            }, 10000);
