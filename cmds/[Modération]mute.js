...
const { MessageEmbed } = require("discord.js");
    ...
    module.exports.run = async(bot, message, args) => {
        if (!message.member.hasPermission('MANAGE_MESSAGES')) return message.channel.send('Vous n\'avez pas la permission **MANAGE_MESSAGES** donc l\'action est impossible !')
        ...
        if(member.id == "911610558682394655") return message.channel.send("_Je ne peux pas **Mute** mon maître, désolé..._ ")
        ...
        let muteRole = message.guild.roles.cache.find(role => role.name === 'Mute By Darkyo#0001')
        if (!muteRole) {
