...
module.exports.run = async (client, message, args) => {
    ...
    try {
      ...
      if(e.status === 403) return message.channel.send(":x: Cet utilisateur en mode privé ou compte supprimé")
      else if(e.status === 404) return message.channel.send(":x: Pas trouvé ")
      else return message.channel.send(`:x: Erreur inconnue:  \`${e.message}\``)
    }
  }
